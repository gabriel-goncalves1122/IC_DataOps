...
# ==========================================
NOME_ARQUIVO_EXCEL = 'MAPBIOMAS_BRAZIL-COVERAGE_STATISTICS-COL.10.1-QUILOMBOS_STATE_BIOME.xlsx'
NOME_ABA_COBERTURA = 'COVERAGE_QUILOMBOS'
NOME_COLUNA_TERRITORIO = 'quilombo_territory' 
NOME_ARQUIVO_SAIDA = 'dim_mapbiomas_quilombos.csv'
...
def limpar_nome_quilombo(texto):
    """Remove códigos entre parênteses. Ex: 'Rio Andirá (281)' -> 'RIO ANDIRA'"""
    if pd.isna(texto): return ""
    nome_puro = str(texto).split('(')[0].strip()
    return remover_acentos(nome_puro)

print(f"Lendo base de Quilombos MapBiomas: {NOME_ARQUIVO_EXCEL}")
...
df_recent['nome_quilombo_limpo'] = df_recent[NOME_COLUNA_TERRITORIO].apply(limpar_nome_quilombo)
...
# Seleção das colunas limpas para o banco
ordem_colunas = [
    'nome_quilombo_limpo', 'estado_norm', 'bioma_norm', 
...
print(f"Sucesso! Dimensão Quilombos gerada com nomes limpos para {len(df_final)} registros.")